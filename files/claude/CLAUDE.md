...
Always run `nvm use` before executing any node/npm/npx commands:
...
**IMPORTANT:** You must use fish shell. When using the Bash tool to run commands, always prefix them with `fish -c` to ensure they run in the correct shell environment.

Example:
```bash
fish -c "nvm use && npm test"
```
